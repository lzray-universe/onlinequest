# 15008 鳍游龙的聚会

出发前的准备都已经完成，是时候前往与玛拉妮约定好的地点了…


---
## 任务信息

- **任务类型**：LQ
- **所属章节**：神秘岛之旅
- **所属地区**：纳塔
- **推荐等级**：28
- **前置主任务**：15007
- **后接主任务**：15009
- **任务奖励**：
  - 冒险阅历×400，摩拉×34075，大英雄的经验×4，精锻用魔矿×7

---
## 剧情流程（按子任务顺序）


### 步骤 1：子任务 1500801

前往约定好的地点

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500801,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_350200053:-2716.443,256.289,8806.695,358.648,352.983,0.159]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 1500801

**起点节点 `150080101`**


**玛拉妮：**#{NICKNAME}还有派蒙，这边！

**派蒙：**玛拉妮已经到了呀，呃，那个怪大叔也在。

**卡嘉：**早上好，两位。

**派蒙：**早上好！

**维兰德：**我们什么时候出发？

**玛拉妮：**哈哈，就像个小孩子一样，在旅行开始前总是迫不及待。

**维兰德：**我是想尽快开始办正事…你肯定也知道这不是什么轻松的春游，我希望我们彼此都能拿出一点专业的态度。

**玛拉妮：**不用担心，我作为向导很专业的。咳咳…那么，我们的神秘岛之旅正式开始了哦。

**派蒙：**好耶！

**玛拉妮：**首先，大家看到这座雕像了吗？

**派蒙：**嗯嗯，它是什么重要的线索吗？

**玛拉妮：**倒不是线索啦，它是我们「流泉之众」的初代首领从神秘岛归来后立在这里的，算是个纪念地标。

**玛拉妮：**出发前首先要做的事呢，就是触摸这座雕像，向先祖祈福，这样我们前往神秘岛的旅途就会更加顺利哦。

**维兰德：**…这种说法能找到什么理论依据吗？

**玛拉妮：**理论依据？没有。但我老师教过我，启程前往神秘岛之前，这是必要的一步。

**维兰德：**……

**玛拉妮：**很好，你没有在自己不专业的领域指手画脚，这也是一种很专业的表现哦。

**维兰德：**好吧，只是这样就没法写进论文里了…

**玛拉妮：**大家一起来！

**玛拉妮：**在此向先祖致意，后辈将踏上您走过的旅途，见证您曾完成的壮举，在您的注视下出发，在您的守望中归来。

**派蒙：**真的有种要开启一场伟大的冒险的感觉呢！

**玛拉妮：**接下来请做好准备，我们要去寻找第一个线索咯。




### 步骤 2：子任务 1500802

前往玛拉妮指示的地点

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500802,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500807,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "0",
        "1"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_BATCH_SET_QUEST_VAR"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_350200055:-2530.570,202.246,8438.508,0.000,247.610,0.000]",
      "15"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 1500802

**起点节点 `150080201`**


**派蒙：**哇，前面有好多鳍游龙啊。

**维兰德：**果然，第一个线索和鳍游龙有关。

**玛拉妮：**部族外的人拿到地图的复制品后，大多能推断出这个结论，但他们找的方向往往是错误的。

**玛拉妮：**初代首领制作地图时在这个环节留了一个特殊的记号，那是我们部族用来标识鳍游龙群的暗号。

**玛拉妮：**所以，随便找一两头落单的鳍游龙可拿不到线索。

**派蒙：**玛拉妮果然很专业！

**维兰德：**可是根据我查到的资料，除非为了争夺温泉池，鳍游龙根本不喜欢群居…

**玛拉妮：**那就告诉你一个只有「流泉之众」的向导才知道的秘密吧，野外的鳍游龙每隔一段时间就会聚集到前面的山谷里。

**玛拉妮：**就像是…部族里定期举办的聚会一样。不过，时节一过，它们就会迅速离开。

**玛拉妮：**如果不是经常在纳塔来回赶路的向导，很难看到这种景象。

**维兰德：**竟然有这种事？得赶快记录下来，必须在论文的相关论述里进行说明…

**玛拉妮：**根据我对地图的解读，线索就藏在那个山谷的深处。

**派蒙：**那要过去的话，岂不是得先穿过龙群？

**玛拉妮：**嗯…只能想办法悄悄溜过去，尽量不要惊动它们。它们应该是在跟朋友聚会，肯定不想被打扰。

**卡嘉：**要主动钻到龙群里？这…

**维兰德：**咳咳…既然决定合作，那双方都该出一份力。不过，卡嘉她之前刚刚被这种鳍游龙袭击过，不方便来做这种事。

**维兰德：**我的话…没什么问题！就让卡嘉在这里等候，原本要交给她的任务一并由我来完成就行。

**玛拉妮：**噗…本来就不需要所有人一起去。

**玛拉妮：**#我们正好需要有人留下来照看行李，这个就拜托两位了，{NICKNAME}和派蒙跟我来就好。

**维兰德：**是这样吗？那自然最好。

**玛拉妮：**不过，你关爱后辈的心思我已经领会到了哦。



#### 对话 1500807

**起点节点 `150080219`**


**玛拉妮：**#我们正好需要有人留下来照看行李，这个就拜托两位了，{NICKNAME}和派蒙跟我来就好。

（缺失节点 `150080220`）



**起点节点 `150080701`**


**[黑屏]** 你们在玛拉妮的带领下来到一处山谷前，而成群的鳍游龙聚集在那里，必须小心翼翼地前进…




### 步骤 3：子任务 1500803

尝试深入山谷

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500803,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_ANY_MANUAL_TRANSPORT",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        3,
        80
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LEAVE_SCENE_RANGE",
      "PPFLKMOMKAI": "[3_350200056:-2526.109,209.219,8373.845]"
    }
  ],
  "execActions": [
    {
      "LNHLPKELCAL": [],
      "PAINLIBBLDK": "QUEST_EXEC_UNLOCK_MIRROR_AVATAR_TEAM"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "1087"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1500806

**起点节点 `150080601`**


**维兰德：**关爱后辈？应该说是针对现状合理安排人手吧？

**卡嘉：**我和前辈会照看好行李，那边就拜托你们了。




### 步骤 4：子任务 1500804

与大家商量对策

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500803,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_350200057:-2518.566,203.578,8446.526,0.000,188.205,0.000]",
      "20"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 1500803

**起点节点 `150080301`**


**派蒙：**这些鳍游龙好警觉啊，这样根本进不去吧？

**维兰德：**既然已经根据地图提供的信息推断出线索在那个山谷里，那直接把这些龙赶走不就好了？阁下的身手应该都很厉害吧？

**玛拉妮：**首先，部族里的前辈一直告诫我，绝对不能主动伤害鳍游龙，那是一等一不吉利的事情。

**玛拉妮：**另外，私自闯入别人的聚会就算了，还要赶别人走？这种行为真的很恶劣哎！

**维兰德：**这算是什么理论…

**卡嘉：**那我们想想其他办法？

**玛拉妮：**嗯…肯定有办法可以在不惊动它们的同时进入山谷。只靠我一个人想办法肯定要很久，我想麻烦大家一起出出点子。

**派蒙：**让我想想…绕路可不可以？

**玛拉妮：**从地图给的信息来看，线索的位置在龙群中间，从另一边走就会遇到另一边的鳍游龙。

**派蒙：**那确实难办了…

**玛拉妮：**…卡嘉姐姐，我发现你总是在帮维兰德圆场的时候才说话，你也有自己的想法吧？能不能讲讲？

**卡嘉：**我吗？好吧…之前维兰德决定去鳍游龙身上找线索，为了帮忙，我也查阅了一些有关鳍游龙的资料。

**卡嘉：**我发现了一个很有趣的地方，我们一向认为鳍游龙依靠听觉来辨别猎物的方向，其实它的嗅觉也十分灵敏。

**卡嘉：**不过，我也不知道这一点有没有用…

**玛拉妮：**哦…哦！很有用的，卡嘉姐姐！

**卡嘉：**叫我卡嘉就好…

**玛拉妮：**#那就麻烦你们继续留在这里看好行李，{NICKNAME}和派蒙跟我来，我想到办法了！




### 步骤 5：子任务 1500805

跟随玛拉妮寻找潜入的办法

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500805,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        121042,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_OBTAIN_ITEM",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "121042"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_DEL_ALL_SPECIFIC_PACK_ITEM"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "1087"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1500809

**起点节点 `150080901`**


**维兰德：**直接驱赶那些鳍游龙不就好了？



#### 对话 1500810

**起点节点 `150081001`**


**卡嘉：**辛苦你们了，我会看好行李的。




### 步骤 6：子任务 1500806

重新尝试潜入

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500804,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500808,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "1",
        "1"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_BATCH_SET_QUEST_VAR"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_350200061:-2527.364,200.950,8480.427,0.000,283.818,0.000]",
      "20"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 1500804

**起点节点 `150080401`**


**玛拉妮：**找到了！

**派蒙：**这是…

**玛拉妮：**这是我们进入鳍游龙聚会的钥匙！

**玛拉妮：**如果鳍游龙的嗅觉也十分敏锐，那么即便我们努力隐藏脚步声，躲避视线，只要一靠近还是会被发现。

**玛拉妮：**而这种植物正好可以解决这种问题，因为它的气味很接近鳍游龙的体味。

**玛拉妮：**用它提取一些汁液，涂抹在身上，就可以骗过鳍游龙的鼻子了！

**派蒙：**要涂在身上啊…

**玛拉妮：**没关系，其实有美容效果的。

**[黑屏]** 玛拉妮采集了一些汁液，给每个人都穿上了一层气味的伪装。

**派蒙：**嗯…湿湿滑滑的，闻起来确实很像鳍游龙呢。

**玛拉妮：**完美，我们再去试一下吧！



#### 对话 1500808

**起点节点 `150080409`**


**[黑屏]** 玛拉妮采集了一些汁液，给每个人都穿上了一层气味的伪装。

**派蒙：**嗯…湿湿滑滑的，闻起来确实很像鳍游龙呢。

**玛拉妮：**完美，我们再去试一下吧！



#### 对话 1500809

**起点节点 `150080901`**


**维兰德：**直接驱赶那些鳍游龙不就好了？



#### 对话 1500810

**起点节点 `150081001`**


**卡嘉：**辛苦你们了，我会看好行李的。




### 步骤 8：子任务 1500807

深入山谷

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500807,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500807,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_NOT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        3,
        120
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LEAVE_SCENE_RANGE",
      "PPFLKMOMKAI": "[3_350200422:-2554.000,203.851,8367.000]"
    }
  ],
  "execActions": [
    {
      "LNHLPKELCAL": [],
      "PAINLIBBLDK": "QUEST_EXEC_UNLOCK_MIRROR_AVATAR_TEAM"
    },
    {
      "LNHLPKELCAL": [
        "133502022"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_UNREGISTER_DYNAMIC_GROUP"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "1087"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1500809

**起点节点 `150080901`**


**维兰德：**直接驱赶那些鳍游龙不就好了？



#### 对话 1500810

**起点节点 `150081001`**


**卡嘉：**辛苦你们了，我会看好行李的。




### 步骤 9：子任务 1500808

观察石壁上的涂鸦

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500805,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "121042"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_DEL_ALL_SPECIFIC_PACK_ITEM"
    },
    {
      "LNHLPKELCAL": [
        "133502675"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_UNREGISTER_DYNAMIC_GROUP"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_350200176:-2513.491,206.290,8311.527,0.000,121.786,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 1500805

**起点节点 `150080501`**


**玛拉妮：**大概就在这附近了。

**派蒙：**看，好多鳍游龙都朝那块岩石聚集过去了，它们这是要干嘛？

**玛拉妮：**嗯…聚会开始显得无聊的时候，就把大家聚到一起，讲一些开心的事情？可惜我听不懂鳍游龙说的话。

**派蒙：**哎？我还以为你们「流泉之众」的向导都能听懂鳍游龙的语言呢。

**玛拉妮：**那你肯定是被人骗了。

**派蒙：**好吧，现在想想确实如此…嗯？那是什么？石壁上出现了…涂鸦？

**玛拉妮：**我确实听说过有一种变色苔藓可以对鳍游龙释放的生物信号起反应，但要足量的话，需要很多很多鳍游龙…

**玛拉妮：**这就是线索！

**派蒙：**要拍下来吗？

**玛拉妮：**闪光会打扰到它们，让我临摹下来吧。

**[黑屏]** 将涂鸦临摹下来之后，玛拉妮带领你们悄悄地原路返回，并将所见所闻告知了等待许久的维兰德和卡嘉。

**派蒙：**这样的话，第一关就算完美通过了吧？

**维兰德：**原来如此，如果一开始选择驱赶那些龙的话，根本拿不到这个线索…果然还是可以用理论来解释。

**玛拉妮：**卡嘉姐姐，他一直这样吗？

**卡嘉：**维兰德他只是太醉心于研究了…

**玛拉妮：**不过这至少说明我的判断没错，你可以信赖我，不是吗？下一次，大家就不用再争执了。

**维兰德：**…我明白，这样确实可以避免浪费时间。

**派蒙：**玛拉妮真的很会带队呢！

**旅行者：**毕竟她是专业的向导。

**玛拉妮：**接下来，就得利用这个新的线索找到下一步要去的地方了，这个图案可能代表什么呢？

**派蒙：**好抽象啊，反正我什么也看不出来。

**玛拉妮：**…哈！是那里！

**玛拉妮：**这几段线条虽然很简陋，但我还是能看出它们是「玉裙之丘」的水陆交界线，只要放到地图上的空缺里…

**玛拉妮：**下一个目的地有了！

**派蒙：**这都能看出来？

**玛拉妮：**其实有直觉的因素在，可能是作为向导，地图看多了，对这种图案比较敏感吧。

**玛拉妮：**准备动身吧，休息的时间结束咯。



#### 对话 1500809

**起点节点 `150080901`**


**维兰德：**直接驱赶那些鳍游龙不就好了？



#### 对话 1500810

**起点节点 `150081001`**


**卡嘉：**辛苦你们了，我会看好行李的。




### 步骤 10：子任务 1500809

(test)用来播黑屏的任务$HIDDEN

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500807,
        3
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_QUEST_STATE_EQUAL",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1500809,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_NOT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [],
    "NOCCOBOFKHP": 0,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NONE"
  }
}
```

（此步骤无对话）
