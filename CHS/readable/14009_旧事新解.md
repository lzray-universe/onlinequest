# 14009 旧事新解

从娜维娅带来的嫌犯口中问出了主谋的名字，但这个人此刻正在梅洛彼得堡，看来需要亲自跑一趟那里…


---
## 任务信息

- **任务类型**：LQ
- **所属章节**：往日留痕
- **所属地区**：枫丹
- **推荐等级**：40
- **任务奖励**：
  - 冒险阅历×550，原石×60，摩拉×38100，「公平」的指引×5，大英雄的经验×4，精锻用魔矿×8

---
## 剧情流程（按子任务顺序）


### 步骤 1：子任务 1400901

前往梅洛彼得堡

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400901,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133401629,1;133401631,1"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_REFRESH_GROUP_SUITE"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_340800121:4449.798,122.395,4617.064,0.000,260.924,0.000]",
      "20"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 1400817

**起点节点 `140081701`**


**娜维娅：**将剩下的「客人」送回去吧，辛苦你们了。

**泰托：**是，老板。



#### 对话 1400818

**起点节点 `140081801`**


**乌迪诺：**呼…逃过一劫。

**梅瑟娜：**这真的算逃过一劫吗？



#### 对话 1400820

**起点节点 `140081901`**


**阿鲁埃：**那维莱特大人亲自光临，这可是绝佳的宣传机会。

**阿鲁埃：**要怎么利用这个机会，进一步扩大知名度呢…



#### 对话 1400902

**起点节点 `140090201`**


**莫莱妮：**是我眼花了吗？




### 步骤 2：子任务 1400902

与莱欧斯利对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400901,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "1078"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1400901

**起点节点 `140090101`**


**莱欧斯利：**欢迎诸位莅临梅洛彼得堡。

**派蒙：**打扰了，我们又来啦！

**派蒙：**没想到是公爵亲自迎接，真有面子，嘿嘿。

**莱欧斯利：**又不是第一次了。你们二位，再加上最高审判官亲自到场，一定是有要事。

**莱欧斯利：**大家就当已经寒暄过了，切入正题吧。

**[黑屏]** 对莱欧斯利说了美露莘遭受威胁的事…

**莱欧斯利：**……

**莱欧斯利：**这个叫多梅尼科的人我有点印象，他曾经试图怂恿并联合犯人们闹事。

**派蒙：**好猖狂的人！没出什么大事吧？

**莱欧斯利：**「曾经试图」，也就是还没开始就结束了。他还威胁过美露莘？

**莱欧斯利：**这个人不是那种能干大事的人，竟然劳烦你们亲自调查，难道背后还有隐情？

**派蒙：**他问了和我们一样的问题！

**那维莱特：**曾经发生过类似的事情，所以我有些担心。我希望见一见多梅尼科，当面问清楚。

**莱欧斯利：**这个简单。我想想，记得他是关在…

**希格雯：**我刚刚好像听到「美露莘」，发生什么事了吗？

**派蒙：**护士长！

**旅行者：**名叫基娅拉的美露莘收到威胁信。

**派蒙：**是呀，我们正在调查这件事，威胁她的人似乎被关在梅洛彼得堡。

**希格雯：**真的吗？基娅拉没事吧？

**那维莱特：**不必担心，克洛琳德正在保护她。

**希格雯：**太好了。不过幕后黑手还在…公爵，你能不能想想办法？

**莱欧斯利：**听护士长的意思，是想要我亲自解决对方吗？

**那维莱特：**莱欧斯利。

**莱欧斯利：**明白，把多梅尼科带过来，只有这一个要求，对吗？

**那维莱特：**算是请求吧。这次我以个人名义拜访，不是正式提审犯人，你无需顾虑。

**那维莱特：**麻烦你了，算我欠你一个人情。

**派蒙：**哇…我要是公爵，就趁机要点什么大的…

**莱欧斯利：**如果我还是犯人，可能会提出希望减刑吧，不过那维莱特先生一定会拒绝。

**莱欧斯利：**不说笑了，我去把多梅尼科找来。

**莱欧斯利：**各位先随意休息一会儿吧。护士长总是惦记你们，一定有很多话想聊。



#### 对话 1400902

**起点节点 `140090201`**


**莫莱妮：**是我眼花了吗？




### 步骤 3：子任务 1400903

到医务室休息

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400906,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_340800122:4576.548,153.710,4633.383,0.000,186.694,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 1400903

**起点节点 `140090301`**


**戈尔代：**应该是我看错了吧…



#### 对话 1400904

**起点节点 `140090401`**


**米勒洛：**嗯？请问有什么事吗？



#### 对话 1400905

**起点节点 `140090501`**


**欧莉艾尔：**我是互助会成员欧莉艾尔，有什么能帮你们的吗？



#### 对话 1400906

**起点节点 `140090601`**


**希格雯：**大家先在这里休息一下吧。

**希格雯：**#那维莱特先生、{NICKNAME}还有派蒙，你们想喝点什么？

**派蒙：**嗯…我还不太渴。谢谢你啦，护士长。

**希格雯：**感到渴的时候，说明身体已经在发出警示，在那之前就要及时补充水分。

**希格雯：**等下估计还有很多事要聊，我先帮你们倒点茶。

**派蒙：**护士长还是那么关心大家的身体健康…

**派蒙：**#咦，那维莱特、{NICKNAME}，你们怎么都不说话了？

**旅行者：**刚才我们路过的那几位罪人…

**那维莱特：**嗯，你也注意到了吗？他们胸口佩戴的徽章，看上去有些眼熟。

**派蒙：**眼熟？呃…我完全没有注意到。

**派蒙：**不过既然你们都这么说，那我们去问问吧！

**分支 1：**

  **旅行者：**我和派蒙去吧。

  **那维莱特：**也好，那我留在这里，等你们消息。

**分支 2：**

  **旅行者：**最高审判官在这里闲逛不太好。

  ↩ 合流到节点 `140090614`




### 步骤 4：子任务 1400904

调查神秘小帮派

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400908,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400909,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "1400907",
        "1"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_ADD_QUEST_PROGRESS"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "13508"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1400908

**起点节点 `140090801`**


**戈尔代：**找我有什么事吗？

**旅行者：**你佩戴的胸章…

**戈尔代：**哦，你说这个啊。你对我们「互助会」感兴趣吗？

**派蒙：**互助会是什么？

**戈尔代：**我们互助会有着非常悠久的历史，最早可以追溯到四百多年前。

**戈尔代：**虽然每代人数都不多，也没什么影响力，但互助会的成员彼此都很友善。

**戈尔代：**互助会的宗旨就是相互帮助，同时尽可能去维护公正。

**派蒙：**听上去是一个很不错的组织！

**戈尔代：**呵呵，我们的初衷只是自保而已。我们没犯过重罪，也没有强大的力量，在梅洛彼得堡中天然处于劣势。

**戈尔代：**只有抱起团来，才不会受到欺负。



#### 对话 1400909

**起点节点 `140090901`**


**戈尔代：**找我有什么事吗？

**旅行者：**你佩戴的胸章…

**戈尔代：**哦，你说这个啊。你对我们「互助会」感兴趣吗？

**派蒙：**互助会？该不会是「檐帽会」那种奇怪的组织吧？

**戈尔代：**檐帽会…你是说前段时间被解散的那个帮派？放心，我们和他们完全不同。

**派蒙：**那我就放心了，互助会平时会做些什么？

**戈尔代：**我们互助会有着非常悠久的历史，最早可以追溯到四百多年前。

**戈尔代：**虽然每代人数都不多，也没什么影响力，但互助会的成员彼此都很友善。

**戈尔代：**互助会的宗旨就是相互帮助，同时尽可能去维护公正。

**派蒙：**听上去是一个很不错的组织！

**戈尔代：**呵呵，我们的初衷只是自保而已。我们没犯过重罪，也没有强大的力量，在梅洛彼得堡中天然处于劣势。

**戈尔代：**只有抱起团来，才不会受到欺负。



#### 对话 1400912

**起点节点 `140091201`**


**戈尔代：**前段时间有个叫多梅尼科的犯人，煽动罪人们起来反抗那维莱特。

**戈尔代：**我们互助会看不下去，把他教训了一顿。




### 步骤 5：子任务 1400905

调查神秘小帮派

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400910,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "1400907",
        "1"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_ADD_QUEST_PROGRESS"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "5272"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1400910

**起点节点 `140091001`**


**米勒洛：**你们对互助会有兴趣吗？

**米勒洛：**这是我们的宣传册，你们可以了解一下。

**米勒洛：**互助会的标语换过很多次，但是核心思想从创立以来一直没有变过。

**米勒洛：**这本书就送给你们啦，如果你们想加入互助会，欢迎来找我报名。



#### 对话 1400913

**起点节点 `140091301`**


**米勒洛：**怎么样，要报名吗？




### 步骤 6：子任务 1400906

调查神秘小帮派

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400911,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "1400907",
        "1"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_ADD_QUEST_PROGRESS"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "13510"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1400911

**起点节点 `140091101`**


**欧莉艾尔：**嗯…总觉得你们看着有些眼熟…

**旅行者：**你佩戴的胸章…

**欧莉艾尔：**这是我们互助会的标志，是初代会长设计的。

**欧莉艾尔：**据说它参考了一种叫「和平勋章」的东西，不过我也没见过和平勋章长什么样子。

**旅行者：**「和平勋章」？

**欧莉艾尔：**初代会长是一个很厉害的人。他很强大，却从来不会欺凌别人，反而鼓励弱小的罪人团结在一起，相互帮助。

**欧莉艾尔：**…我们都很尊敬他。



#### 对话 1400914

**起点节点 `140091401`**


**欧莉艾尔：**在加入互助会之前，我也有许多怨言。虽然知道自己犯了罪，但日复一日生活在这里，还是很痛苦。

**欧莉艾尔：**反倒是现在，和其他同伴一起聊天、做事，生活也有了色彩。




### 步骤 7：子任务 1400907

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 3,
      "LNHLPKELCAL": [
        1400907,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_ADD_QUEST_PROGRESS",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "1400908",
        "1"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_ADD_QUEST_PROGRESS"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [],
    "NOCCOBOFKHP": 0,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NONE"
  }
}
```

（此步骤无对话）


### 步骤 8：子任务 1400908

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        1400908,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_ADD_QUEST_PROGRESS",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400904,
        3
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_QUEST_STATE_EQUAL",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400905,
        3
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_QUEST_STATE_EQUAL",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400906,
        3
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_QUEST_STATE_EQUAL",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [],
    "NOCCOBOFKHP": 0,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NONE"
  }
}
```


#### 对话 1400907

**起点节点 `140090701`**


**那维莱特：**我对莱欧斯利很放心，所以并不会经常来梅洛彼得堡。这次的情况比较特殊，希望没有给他造成什么困扰。



#### 对话 1400912

**起点节点 `140091201`**


**戈尔代：**前段时间有个叫多梅尼科的犯人，煽动罪人们起来反抗那维莱特。

**戈尔代：**我们互助会看不下去，把他教训了一顿。



#### 对话 1400913

**起点节点 `140091301`**


**米勒洛：**怎么样，要报名吗？



#### 对话 1400914

**起点节点 `140091401`**


**欧莉艾尔：**在加入互助会之前，我也有许多怨言。虽然知道自己犯了罪，但日复一日生活在这里，还是很痛苦。

**欧莉艾尔：**反倒是现在，和其他同伴一起聊天、做事，生活也有了色彩。




### 步骤 9：子任务 1400909

将调查结果告诉那维莱特

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400915,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "1076"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1400912

**起点节点 `140091201`**


**戈尔代：**前段时间有个叫多梅尼科的犯人，煽动罪人们起来反抗那维莱特。

**戈尔代：**我们互助会看不下去，把他教训了一顿。



#### 对话 1400913

**起点节点 `140091301`**


**米勒洛：**怎么样，要报名吗？



#### 对话 1400914

**起点节点 `140091401`**


**欧莉艾尔：**在加入互助会之前，我也有许多怨言。虽然知道自己犯了罪，但日复一日生活在这里，还是很痛苦。

**欧莉艾尔：**反倒是现在，和其他同伴一起聊天、做事，生活也有了色彩。



#### 对话 1400915

**起点节点 `140091501`**


**那维莱特：**打听到什么了吗？

**派蒙：**那些佩戴徽章的人，都加入了一个叫互助会的组织。

**派蒙：**他们说这个徽章是参考「和平勋章」设计的。那维莱特，你见过和平勋章吗？

**那维莱特：**我曾经亲手做过两枚和平勋章，颁给卡萝蕾与沃特林。

**那维莱特：**卡萝蕾那枚在烈火中烧尽了，只剩下沃特林手中还有一枚。

**派蒙：**啊…

**派蒙：**这么说，互助会的创始人其实是——

**希格雯：**那维莱特先生，公爵请你们去办公室，他已经找到多梅尼科了。

**那维莱特：**…好。我们先去见见他，把美露莘的事问清楚。




### 步骤 10：子任务 1400910

前往莱欧斯利办公室

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1086,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_ENTER_ROOM",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400910,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "1078"
    ],
    "NOCCOBOFKHP": 1086,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1400912

**起点节点 `140091201`**


**戈尔代：**前段时间有个叫多梅尼科的犯人，煽动罪人们起来反抗那维莱特。

**戈尔代：**我们互助会看不下去，把他教训了一顿。



#### 对话 1400913

**起点节点 `140091301`**


**米勒洛：**怎么样，要报名吗？



#### 对话 1400914

**起点节点 `140091401`**


**欧莉艾尔：**在加入互助会之前，我也有许多怨言。虽然知道自己犯了罪，但日复一日生活在这里，还是很痛苦。

**欧莉艾尔：**反倒是现在，和其他同伴一起聊天、做事，生活也有了色彩。




### 步骤 11：子任务 1400911

与莱欧斯利对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400916,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "1078"
    ],
    "NOCCOBOFKHP": 1086,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1400912

**起点节点 `140091201`**


**戈尔代：**前段时间有个叫多梅尼科的犯人，煽动罪人们起来反抗那维莱特。

**戈尔代：**我们互助会看不下去，把他教训了一顿。



#### 对话 1400913

**起点节点 `140091301`**


**米勒洛：**怎么样，要报名吗？



#### 对话 1400914

**起点节点 `140091401`**


**欧莉艾尔：**在加入互助会之前，我也有许多怨言。虽然知道自己犯了罪，但日复一日生活在这里，还是很痛苦。

**欧莉艾尔：**反倒是现在，和其他同伴一起聊天、做事，生活也有了色彩。



#### 对话 1400916

**起点节点 `140091601`**


**莱欧斯利：**向大家介绍，这位好先生就是多梅尼科。

**莱欧斯利：**你不如先交代一下？

**多梅尼科：**那、那维莱特？！

**多梅尼科：**你怎么会在这里？你想干什么？灭口吗？

**那维莱特：**冷静。我只是想问你几个问题。

**那维莱特：**给美露莘寄送威胁信是你的主意吗？

**多梅尼科：**……

**那维莱特：**埃桑已经坦白了，在这件事上你没有必要隐瞒。

**多梅尼科：**那个混蛋——枉我那么信任他！

**那维莱特：**我再问一遍：给美露莘送威胁信是你的主意，对吗？

**多梅尼科：**…是。

**那维莱特：**你背后是谁？

**多梅尼科：**什么？

**多梅尼科：**你——你们该不会是想把什么案件栽赃到我头上吧？

**那维莱特：**……

**莱欧斯利：**认清自己的处境，最高审判官正在亲自调查你的案件。

**莱欧斯利：**前不久你试图在梅洛彼得堡聚众闹事，应该也与此有关吧。

**多梅尼科：**我…我承认我是有点冲动…我可以说实话，但你们要保证我的人身安全。

**那维莱特：**我可以保证。

**莱欧斯利：**放心，我们是很讲道理的。

**多梅尼科：**我…最初只是做点小生意，有人找我帮他们「运货」，并且承诺给我一大笔钱。

**多梅尼科：**运了几趟之后，逐影庭突然找到了我，说货物是违禁品。我被抓了起来，然后遭受了审判——

**多梅尼科：**但是我不服，不是有句话叫「不知者不罪」吗？我，我怀疑有人故意把我流放到这里，好贪掉我手上的货…

**那维莱特：**……

**莱欧斯利：**……

**派蒙：**该不会…你就是因为这样的理由，决定报复美露莘？

**分支 1：**

  **旅行者：**「不知者不罪」不是借口。

  **多梅尼科：**我原本的目标是那维莱特，梅洛彼得堡的罪人都是他审判的。那些罪人们一定有许多不满，如果能联合他们闹事…

  **多梅尼科：**但是不知为何，并没有人愿意配合我。尤其是那个互助会，一帮小角色而已，居然也敢和我叫板…

  **多梅尼科：**我左思右想，只能从美露莘身上找回场子。

  **多梅尼科：**我想起当初就是那个基娅拉带人查封了我的货，所以就找了个好说话的看守，恳求他帮我送一封家书出去…

  **多梅尼科：**但信中其实是让埃桑给基娅拉写威胁信，想逼迫基娅拉退出逐影庭…

  **那维莱特：**以上这些，都是你自己的想法是吗？这期间你有没有接触什么可疑的人？

  **多梅尼科：**没有。

  **那维莱特：**委托你运货的人，真的全部落网了吗？

  **多梅尼科：**是的…我只知道这么多了。

  **那维莱特：**……

  **莱欧斯利：**…护士长，你先把多梅尼科带回看守那边吧，等会儿再处置他。

  **莱欧斯利：**所以，那维莱特先生原本是担心有某种势力想借对付美露莘来制造更大的风波，对吗？

  **那维莱特：**嗯。我经历过相似的事情，不得不防范这种可能性。

  **莱欧斯利：**所谓「相似的事情」，大概是多久之前的事了？

  **那维莱特：**四百多年前。

  **莱欧斯利：**我觉得是你多虑了。

  **莱欧斯利：**时间会改变很多东西，如今一切都不同了。

  **那维莱特：**你是指什么？

  **莱欧斯利：**四百多年前，无论是你，还是你带来的美露莘，对枫丹的人类社会来说都是异类。

  **莱欧斯利：**但时至今日，如果有人威胁美露莘，人们应该不会坐视不管，他们会做出与从前不同的选择。

  **派蒙：**的确。我们回沫芒宫的时候，看到很多人自发地保护美露莘呢！

  **莱欧斯利：**那维莱特，美露莘是你亲自带到枫丹的种族。民众对待她们的态度，其实反映了社会对你的看法。

  **莱欧斯利：**过去的民众不信任美露莘，本质上也是因为他们在观望你这位外来的最高审判官。

  **莱欧斯利：**将近五百年的时间，你公正地执行了每一场审判。无论法庭中上演了怎样的闹剧，你都做出了正确的裁决。

  **莱欧斯利：**如今民众不再对你有任何疑虑，甚至视你为律法的象征。

  **莱欧斯利：**现在你所做的每个决定都会影响到整个枫丹，或者说，你已经潜移默化地改变了这个国家。

  **派蒙：**原来是这样，所以才没有多少人愿意与多梅尼科合作…

  **旅行者：**当初的悲剧已经不会再发生了。

  **那维莱特：**这份称赞对我来说太过沉重。在我看来，我只是在完成自己的「职责」而已。

  **那维莱特：**不值得夸耀，也没什么特别。我是在兑现过去的承诺，同时借审判来寻找答案。

  **那维莱特：**我不该得到这样的「尊敬」…人类的情感与意志比我复杂许多，坦率地说，我认为自己更需要尊敬你们。

  **莱欧斯利：**不必如此谦虚，现状已经说明了一切。

  **莱欧斯利：**你已经不是当初那个外来者了，现在的你就算想单独调查一件事，也会有许多和你有关的人主动参与进来。

  **那维莱特：**有道理，至少这一点我无法反驳。

  **那维莱特：**感谢你为我解惑。事情顺利解决，我也准备告辞了。

  **派蒙：**咦！现在就走吗？

  **莱欧斯利：**这就走了，不留下喝杯茶？

  **那维莱特：**不必客气，我知道规矩的。

  **那维莱特：**这里有诚心赎罪的人，但也肯定有对我心怀怨恨的人。

  **那维莱特：**我在这里停留的时间越短越好，不然可能会引发骚乱。

  **莱欧斯利：**那看来只能由我来「违规」了。还有两件小事。

  **那维莱特：**请讲。

  **莱欧斯利：**一个是关于帮多梅尼科送信的那名看守…

  **那维莱特：**啊，我明白了。

  **那维莱特：**看守虽然是沫芒宫派来的人员，但我也说过这次拜访算是我的私事。交给你处理吧。

  **莱欧斯利：**那就好办了。

  **旅行者：**你是想…

  **莱欧斯利：**没有，别想夸张了。

  **莱欧斯利：**从水上来的人相对来说更容易心生同情，但这种想法在水下的世界里非常危险。

  **莱欧斯利：**我会找那名看守聊聊，强调些注意事项。

  **那维莱特：**我没什么问题。

  **派蒙：**太好了，一件事情解决！那另一件呢？

  **莱欧斯利：**另一件呢，则出于我的个人好奇心。我听说你们刚刚调查了「互助会」的事？

  **派蒙：**是呀，因为那维莱特说他们戴的徽章很眼熟…

  **莱欧斯利：**这个小帮派我接手梅洛彼得堡的时候就注意到了，他们体量不大，但一直在做力所能及的正义之事。

  **莱欧斯利：**我查过他们的创始人——也就是当初接受审判后，被关押在这里的沃特林。

  **莱欧斯利：**根据现存的档案记载，此人在服刑期间一直服从管教，直至离世也从未有过过激行为或言论。

  **莱欧斯利：**——也就是说，他在水下的态度，与他在庭审现场的表现大相径庭。

  **那维莱特：**什么？

  **旅行者：**他不憎恨那维莱特吗？

  **那维莱特：**在审判的时候，我察觉到他的情感很复杂。他看起来充满恨意，也确实有憎恶我的理由。

  **莱欧斯利：**我猜…那也许只是一种表演。

  **派蒙：**表演？

  **莱欧斯利：**那维莱特先生与沃特林曾经是关系紧密的上下级，沃特林一定知道最高审判官必会做出无私心的判决。

  **莱欧斯利：**而他越是表现出憎恨的情感，出席那场审判的人就越能直观地感受和认可你的公正。

  **莱欧斯利：**——对于当时还在观望的民众来说，这很有意义。

  **那维莱特：**……

  **莱欧斯利：**我的两件事说完了。大家还有其他要讲的吗？抓住机会。

  **那维莱特：**我没有要说的了——除了感谢以外。

  **那维莱特：**我们回去吧，不必送了。

  **莱欧斯利：**各位慢走。

**分支 2：**

  **旅行者：**感觉没救了。

  ↩ 合流到节点 `140091629`




### 步骤 12：子任务 1400917

返回枫丹廷

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400917,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_340100365:3634.824,445.895,4511.580,0.000,182.921,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 1400912

**起点节点 `140091201`**


**戈尔代：**前段时间有个叫多梅尼科的犯人，煽动罪人们起来反抗那维莱特。

**戈尔代：**我们互助会看不下去，把他教训了一顿。



#### 对话 1400913

**起点节点 `140091301`**


**米勒洛：**怎么样，要报名吗？



#### 对话 1400914

**起点节点 `140091401`**


**欧莉艾尔：**在加入互助会之前，我也有许多怨言。虽然知道自己犯了罪，但日复一日生活在这里，还是很痛苦。

**欧莉艾尔：**反倒是现在，和其他同伴一起聊天、做事，生活也有了色彩。



#### 对话 1400917

**起点节点 `140091701`**


**莱欧斯利：**人活着就会有烦恼，最高审判官的格外多呢。




### 步骤 13：子任务 1400912

跟随那维莱特

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400912,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400912,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_NOT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        3,
        120
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LEAVE_SCENE_RANGE",
      "PPFLKMOMKAI": "[3_340100250:3593.442,445.895,4456.895,0.000,60.466,0.000]"
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_TEAM_DEAD",
      "PPFLKMOMKAI": ""
    }
  ],
  "execActions": [
    {
      "LNHLPKELCAL": [],
      "PAINLIBBLDK": "QUEST_EXEC_UNLOCK_AVATAR_TEAM_V2"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "1076"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```

（此步骤无对话）


### 步骤 14：子任务 1400913

与那维莱特对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400918,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "5053",
        "0"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_SET_WEATHER_GADGET"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "1076"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1400918

**起点节点 `140091801`**


**派蒙：**嗯…我觉得公爵说的那些很对呀。

**派蒙：**#作为最高审判官，你主持的每一场审判都会影响到枫丹，是这样吧…{NICKNAME}，你觉得呢？

**派蒙：**#喂——{NICKNAME}，你在想什么呀？

**旅行者：**我在想——

**旅行者：**一场又一场的审判，是否也在反过来影响你呢？

**旅行者：**（身为水元素龙王的那维莱特仍然按照固有印象称自己为「局外人」…）

**旅行者：**（但是在经历了如此漫长的时间后，也许他与人类之间的边界早就已经模糊了吧。）

**那维莱特：**…有道理。

**那维莱特：**我说过，我不擅长表露情感…是因为我没办法完全理解自己。

**那维莱特：**不过我相信你的判断。不知从何时开始，我的身上出现了一些「变化」。

**那维莱特：**也许这些改变并不是某个具体的事件造成的，而是来源于时间本身。

**那维莱特：**我会认真思考这个问题，谢谢你们。




### 步骤 15：子任务 1400916

与夏洛蒂对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400925,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "1070"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1400921

**起点节点 `140092101`**


**利弗：**夏洛蒂的声音还是那么有穿透力。虽然不知道发生了什么，但应该是遇到好事了吧。

**克叙涅：**每个人看起来心情都很好，今天是个好日子啊。



#### 对话 1400925

**起点节点 `140092501`**


**克洛琳德：**麻烦解决了吗？

**派蒙：**我们找到寄威胁信的人啦，莱欧斯利说会对他严加看守，现在应该安全了。

**娜维娅：**太好了。

**那维莱特：**很抱歉，把你们也卷进来了。

**克洛琳德：**无需道歉。

**娜维娅：**是啊，与其说卷进来，不如说是我们擅自参与进来的。

**派蒙：**对了，克洛琳德，基娅拉呢？

**克洛琳德：**在沫芒厅。刚刚有超过五十人自愿保护她，甚至有人藏在路边的花丛中警戒。

**克洛琳德：**我担心太多人加入反而会给她造成困扰，所以先将她安置在沫芒厅了，那里很安全。

**派蒙：**呼…那我就放心啦。

**夏洛蒂：**看来事情告一段落了？

**夏洛蒂：**作为庆祝——那维莱特先生有没有兴趣接受一个独家专访？

**派蒙：**没有这种庆祝方式啦！

**那维莱特：**可以考虑。

**夏洛蒂：**啊？真的吗？

**那维莱特：**当然。不过我接下来的日程很满，就定在下个月吧。

**那维莱特：**在那之前，你可以收集一些问题，我会予以必要的解答。

**夏洛蒂：**……

**夏洛蒂：**……

**夏洛蒂：**太好啦！！

**那维莱特：**现在，请允许我先告辞。我准备回沫芒宫签署命令，公开这一次的调查结果。

**那维莱特：**这样民众也不需要再担心了。




### 步骤 16：子任务 1400914

返回沫芒宫

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1083,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_ENTER_ROOM",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400914,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "133401465"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_UNREGISTER_DYNAMIC_GROUP"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[1083_46:-40.014,0.331,-5.642,0.000,175.855,0.000]"
    ],
    "NOCCOBOFKHP": 1083,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 1400919

**起点节点 `140091901`**


**夏洛蒂：**独家专访！我居然不是在做梦…要问些什么呢？

**娜维娅：**不用拘谨！机会难得，大胆问！

**克洛琳德：**同感。反正遇到不能回答的问题他自然会拒绝的，不妨一试。



#### 对话 1400921

**起点节点 `140092101`**


**利弗：**夏洛蒂的声音还是那么有穿透力。虽然不知道发生了什么，但应该是遇到好事了吧。

**克叙涅：**每个人看起来心情都很好，今天是个好日子啊。




### 步骤 17：子任务 1400915

与基娅拉对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        1400922,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "1400901",
        "1"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_SET_QUEST_GLOBAL_VAR"
    },
    {
      "LNHLPKELCAL": [
        "120640"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_DEL_ALL_SPECIFIC_PACK_ITEM"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "13503"
    ],
    "NOCCOBOFKHP": 1083,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 1400922

**起点节点 `140092201`**


**基娅拉：**呼…呼…

**那维莱特：**基娅拉？

**基娅拉：**那维莱特大人？不好意思，我刚刚不小心睡过去啦。

**那维莱特：**没关系。我想告诉你，寄威胁信的人我们已经查到了，你现在很安全。

**基娅拉：**谢谢您，那维莱特大人。

**基娅拉：**#还有{NICKNAME}{M#哥哥}{F#姐姐}和派蒙姐姐，谢谢你们。我一直都觉得自己很安全，大家对我都很好…

**那维莱特：**寄威胁信的人名叫多梅尼科，你还记得这个名字吗？

**基娅拉：**我想想…记不清啦。我记忆力不是很好，经常忘事…

**基娅拉：**比起这个，我刚刚梦到卡萝蕾姐姐了。奇怪，卡萝蕾姐姐去哪了？好像好久没见到她了。

**派蒙：**……

**基娅拉：**不过梦里的她很开心喔，还拉着我的手说：「基娅拉，我们的梦想实现啦！」

**基娅拉：**我想不起来她的梦想是什么了。但是不知道为什么，我也很开心。

**那维莱特：**我能感受到你的喜悦。这确实是一件值得高兴的事情。

**基娅拉：**嘿嘿…那维莱特大人也感到高兴吗？太好了。

**基娅拉：**啊！对了，现在我可以外出了吗？

**基娅拉：**我和爱洛芙姐姐约好了，要去当「模特」呢！

**那维莱特：**当然可以，放心去吧。

**基娅拉：**#那我走啦。下次再见，那维莱特大人、{NICKNAME}{M#哥哥}{F#姐姐}、派蒙姐姐！

**那维莱特：**嗯…下次再见。



#### 对话 1400923

**起点节点 `140092301`**


**那维莱特：**……

**那维莱特：**有时候真是搞不懂她。



#### 对话 1400924

**起点节点 `140092401`**


**「那维莱特的假条」：**「今日外出处理私事，请假一天。」

**「那维莱特的假条」：**「『私事』？哈哈，我们的最高审判官先生终于开窍了。」

**「那维莱特的假条」：**「我早就说过，你也应该像我一样多出去走走，与子民们亲近一些嘛。」

**「那维莱特的假条」：**「期间就算发生一些让人遗憾的事，也一定会遇到许多特别的家伙。」

**「那维莱特的假条」：**「…怎么样，人的世界还算有趣吧？」



#### 对话 1400926

**起点节点 `140092601`**


**那维莱特：**我曾认为时间对我来说是无用之物。

**那维莱特：**与所有人保持距离、妥善地完成工作，几年与几百年并无区别。

**那维莱特：**但…经历了这些，我意识到或许这种想法并不全面。

**那维莱特：**时间的流动自有其意义，我想从人类生活中寻找的答案，说不定就是生活本身。


