# 76004 是严谨派还是悟性派？

在苔骨荒原，发现了两名对如何操控大型机械装置而一筹莫展的宝藏猎人…


---
## 任务信息

- **任务类型**：WQ
- **前置主任务**：76102
- **后接主任务**：76116
- **任务奖励**：
  - 冒险阅历×350，原石×40，摩拉×30000，大英雄的经验×4，精锻用魔矿×4

---
## 剧情流程（按子任务顺序）


### 步骤 1：子任务 7600401

与马科拉他们对话$HIDDEN

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600401,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607001"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "21828"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NPC"
  }
}
```


#### 对话 7600401

**起点节点 `760040101`**


**安妮兹卡：**嗯嗯，就是像这样移动的！真乖真听话！

**马科拉：**安妮兹卡果然非同一般，竟然第一次操作就成功了！

**安妮兹卡：**哼哼，悟性派就是这样的！欸，马科拉，我好像看到有人往这里来了！

**马科拉：**说起来，德兹斯拉沃好厉害！光是介绍各个部件的名字就写了四十多页…

**安妮兹卡：**没错，小德最会起名字啦！不过，你快放下手头那个东西去接待客人吧。

**马科拉：**#这位{M#帅气的先生}{F#可爱的小姐}，要一起来和博比克玩吗？

**旅行者：**博比克…？

**派蒙：**该不会是指这台巨大的机械装置吧…

**马科拉：**没错！<color=#00E1FFFF>博比克-131</color>，全地形民用运输装置，是我们的好朋友德兹斯拉沃的得意之作。

**马科拉：**但他留下的操作说明文档看得我头好痛…不过，头痛就是在长知识了吧！也就是说，我很快就能拥有科学家的头脑了！

**安妮兹卡：**马科拉的身手加上小德的头脑，还有我的悟性！那就再也不用为赏金发愁啦！你说是不是呀博比克？是不是呀博比克？

**[黑屏]** 博比克发出了「咔咔咔——轰轰！」的声音。

**派蒙：**#呜！{NICKNAME}小心，会被踩扁的！

**安妮兹卡：**不用担心哦，博比克是很温柔的孩子，绝对不会伤害人的！

**分支 1：**

  **旅行者：**真是令人安心呢…

  **派蒙：**哪里令人安心了？这种状况看起来怎么都没有办法安心的吧？

  **安妮兹卡：**可是，操控大机器人真的很好玩哦！一开始看到博比克的样子可能会有点被震慑到，但实际上手的体验，实在是无与伦比！

  **派蒙：**#竟然会很好玩吗？说得都有点想试试看了…{NICKNAME}是怎么想的？

  **分支 1：**

    **旅行者：**当然要操作大机器人了！

    **马科拉：**太好了，这样博比克就有新玩伴了！不过在此之前，我有一个很重要的问题想问问——

    **马科拉：**要操控像博比克这样的大型机械，你是啃透厚厚说明文档的严谨派，还是直接开机，用灵魂去感受的悟性派？

    **分支 1：**

      **旅行者：**严谨派。

      **马科拉：**好少见！德兹斯拉沃要是认识你的话也会很高兴的。可惜那个说明文档，他三令五申对我说过，不可以给别人看的…抱歉啦！

      **马科拉：**刚才安妮兹卡已经掌握了博比克的移动机制！但博比克是为了方便地搬运大宗货物这个目的而设计的。

      **马科拉：**至于要怎么搬运货物…现在还没有头绪。不过，要是你能帮忙的话，我就不用花时间啃文档啦！

      **马科拉：**这里有很多没用的大箱子，试试看用博比克把它们摆成有趣的形状吧！

      **安妮兹卡：**嗯，请尽情用灵魂感受博比克！

    **分支 2：**

      **旅行者：**悟性派。

      **马科拉：**跟安妮兹卡一样，是很厉害的人！于是啃文档的人就只剩下了我…果然，成为科学家的道路是孤寂的。

      ↩ 合流到节点 `760040131`

    **分支 3：**

      **旅行者：**派蒙的派。

      **派蒙：**嗯？派蒙的派是什么派啦！真是的…头好痛，刚才不是说头痛就是在长知识吗？好像完全没有感觉到呢…

      **派蒙：**#好吧，既然{NICKNAME}是我的派，那我就要立下规定了！请好好研究博比克的操控方式，这样下去实在太危险了…

      ↩ 合流到节点 `760040131`

  **分支 2：**

    **旅行者：**我来操作的确会更安全一点吧。

    ↩ 合流到节点 `760040122`

**分支 2：**

  **旅行者：**很难不担心呢…

  ↩ 合流到节点 `760040118`



#### 对话 7600408

**起点节点 `760040801`**


**安妮兹卡：**#博比克要乖哦，这个{NICKNAME}是新朋友！

**马科拉：**嗯！没错，要有礼貌！




### 步骤 2：子任务 7600402

操作博比克搬运集装箱

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600402"
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11428:1861.685,240.704,9412.039]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 3：子任务 7600403

与马科拉他们对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600403,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600406,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_11430:1855.844,235.316,9418.246,0.000,166.159,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600403

**起点节点 `760040301`**


**安妮兹卡：**不愧是博比克看中的人，果然很厉害呢！

**马科拉：**不愧是安妮兹卡看中的人，果然很厉害呢！

**派蒙：**#不愧是…等一下，我为什么要加入夸夸队列？都怪{NICKNAME}你刚才用充满期待的眼神盯着我看啦。

**派蒙：**不过，竟然真的靠悟性摸索出了博比克的操作，还没有造成任何人受伤，的确很了不起。

**安妮兹卡：**而且！仔细看这些箱子的排列，很有艺术性不是吗？

**马科拉：**使用重型全地形民用运输装置完成的艺术品…就是「装置艺术」吧！

**安妮兹卡：**原来是这个意思吗？我从前一直都不明白什么是装置艺术，马科拉这么一解释我就完全明白了。

**旅行者：**装置艺术不是这个意思哦。

**派蒙：**没错，说起装置艺术，我们还是有所了解的。毕竟，我们是「打倒邪恶组织守护枫丹未来艺术家联盟」的成员！

**马科拉：**好厉害的名号！原来艺术家们也加入到了打倒邪恶组织的战斗中来了吗？

**安妮兹卡：**我也好想加入！不过我还没有创作过艺术品…对了！只要现在开始创作就好了！

**马科拉：**科学家的头脑加上艺术家的品味…安妮兹卡，没有更合适的人选了吧？

**安妮兹卡：**没错！那么剩下的那些箱子，就请一起帮忙把它们都做成漂亮的艺术品吧！

**派蒙：**原来还有别的箱子吗？不过，如果全都交给他们两位的话，的确让人放不下心来呢…

**旅行者：**那就一起快乐地创作艺术吧！

**安妮兹卡：**好欸！我也要帮忙。

**马科拉：**那么两位就先过去吧，我和安妮兹卡，还有博比克随后就到！



#### 对话 7600406

**起点节点 `760040601`**


**安妮兹卡：**不愧是博比克看中的人，果然很厉害呢！

**马科拉：**不愧是安妮兹卡看中的人，果然很厉害呢！

**派蒙：**#不愧是…等一下，我为什么要加入夸夸队列？都怪{NICKNAME}你刚才用充满期待的眼神盯着我看啦。

**派蒙：**不过，竟然真的靠悟性摸索出了博比克的操作，还没有造成任何人受伤，的确很了不起。

**安妮兹卡：**而且！仔细看这些箱子的排列，很有艺术性不是吗？

**马科拉：**使用重型全地形民用运输装置完成的艺术品…就是「装置艺术」吧！

**安妮兹卡：**原来是这个意思吗？我从前一直都不明白什么是装置艺术，马科拉这么一解释我就完全明白了。

**马科拉：**科学家的头脑加上艺术家的品味…安妮兹卡，没有更合适的人选了吧？

**安妮兹卡：**没错！那么剩下的那些箱子，就请一起帮忙把它们都做成漂亮的艺术品吧！

**派蒙：**原来还有别的箱子吗？不过，如果全都交给他们两位的话，的确让人放不下心来呢…

**旅行者：**那就一起快乐地创作艺术吧！

**安妮兹卡：**好欸！我也要帮忙。

**马科拉：**那么两位就先过去吧，我和安妮兹卡，还有博比克随后就到！



#### 对话 7600408

**起点节点 `760040801`**


**安妮兹卡：**#博比克要乖哦，这个{NICKNAME}是新朋友！

**马科拉：**嗯！没错，要有礼貌！



#### 对话 7600417

**起点节点 `760041701`**


**安妮兹卡：**看看还有什么要带上的？哦对，还有马科拉！

**马科拉：**我是不会跟丢的啦！




### 步骤 4：子任务 7600404

前往指定地点

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600404,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_360700101:1782.126,254.217,9408.031]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600414

**起点节点 `760041401`**


**安妮兹卡：**好好奇呢，博比克这孩子会做出什么样的艺术品来呢？

**马科拉：**#放心吧，有{NICKNAME}帮它，肯定会很棒的！




### 步骤 5：子任务 7600405

操作博比克「创作艺术」

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600405,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_360700101:1782.126,254.217,9408.031]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600405

**起点节点 `760040501`**


**马科拉：**就是这里。接下来的部分我们都不擅长，就交给真正的艺术家发挥好了。



#### 对话 7600417

**起点节点 `760041701`**


**安妮兹卡：**看看还有什么要带上的？哦对，还有马科拉！

**马科拉：**我是不会跟丢的啦！




### 步骤 6：子任务 7600406

操作博比克「创作艺术」

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600406"
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607041"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11461:1784.296,254.140,9411.059]",
      "20"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 7：子任务 7600407

与马科拉他们对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600407,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_11461:1784.296,254.140,9411.059]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600407

**起点节点 `760040701`**


**安妮兹卡：**哇，真好看！博比克好像也很喜欢！

**马科拉：**让我看看接下来该去哪里…

**愚人众特遣队：**原来是在这里嘛，找得我好辛苦。快把东西交给我们愚人众特遣队，接下来就没你们什么事了！

**马科拉：**这些人是谁？是两位认识的人吗？

**分支 1：**

  **旅行者：**完全没有印象。

  **派蒙：**没错，一起旅行了那么长时间，揍过的愚人众根本数不过来，所以完全不认识呢。

  **愚人众特遣队：**哼，无礼的家伙！不过我们不是来动手的，只要乖乖交出陆巡舰，我可以考虑放过你们。

  **派蒙：**陆巡舰——难道是指博比克？这和发明了博比克的科学家说的不一样吧…

  **安妮兹卡：**没错，小德说的很清楚，只要我们保证博比克的用途永远会是民用运输，他就把博比克托付给我们了。

  **愚人众特遣队：**德兹斯拉沃？他不过是受愚人众雇佣而已，在此期间他所有的发明都归我们所有，连名字都不例外！他没有资格转让任何东西。

  **愚人众特遣队：**只要你们现在把陆巡舰交出来，德兹斯拉沃盗取愚人众重要资产逃跑的事，我们也可以网开一面。

  **愚人众特遣队：**众所周知，我们愚人众的势力强大。既然你们自认为是他的朋友，那么就放聪明些！

  **安妮兹卡：**马科拉，怎么会有这么没眼力的人？

  **马科拉：**就是啊，既然德兹斯拉沃把博比克托付给我们，我们当然要好好照顾它了。

  **安妮兹卡：**而且！博比克还这么可爱！简直就跟、跟…小狗一样！

  **马科拉：**嗯！虽然说小狗是小狗，人是人，两者不能相提并论。但在今天这个社会，很多宠物已经被大家当作家人一般珍惜了。

  **马科拉：**我们怎么可能会同意让不认识的人带走它呢？

  **安妮兹卡：**没错！看到别人家的孩子很可爱，就跑过来随便起一个奇怪的名字带走，像这样的家伙，一定要好好教训一番才对！

  **分支 1：**

    **旅行者：**不会让他们带走博比克的。

    **愚人众特遣队：**哼，那就让你们瞧瞧，谁才是陆巡舰真正的主人！

  **分支 2：**

    **旅行者：**博比克是全地形民用运输装置！

    ↩ 合流到节点 `760040722`

**分支 2：**

  **旅行者：**只能认出是愚人众…

  ↩ 合流到节点 `760040707`



#### 对话 7600414

**起点节点 `760041401`**


**安妮兹卡：**好好奇呢，博比克这孩子会做出什么样的艺术品来呢？

**马科拉：**#放心吧，有{NICKNAME}帮它，肯定会很棒的！




### 步骤 8：子任务 7600408

击退愚人众

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600408"
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "133607048",
        "0"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_UNREGISTER_DYNAMIC_GROUP"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11463:1793.111,252.089,9414.685,0.000,277.239,0.000]",
      "20"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 9：子任务 7600409

与马科拉他们对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600409,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_11339:1783.552,253.973,9402.597,0.000,322.243,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600409

**起点节点 `760040901`**


**安妮兹卡：**马科拉你刚才干嘛拦着我，我也想去揍那些家伙几下！

**马科拉：**安妮兹卡不要生气啦。像那种缺爱的家伙，看到我们两个幸福的家庭各自带着孩子以宠会友，大概妒忌得快要发疯了吧。

**派蒙：**#嗯…嗯？「两个幸福的家庭各自带着孩子以宠会友」…你们两个，该不会又以为我是{NICKNAME}的宠物吧？！

**安妮兹卡：**#当然不是！主人和宠物我们怎么会搞错呢？明明{NICKNAME}是派蒙的宠物才对吧！

**旅行者：**？

**派蒙：**这、这倒不是不…等一下，是为什么会得出这种匪夷所思的结论的！

**安妮兹卡：**完美的容貌、漂亮的金色毛发、特立独行的名字…还有！才刚跟博比克认识，就能快乐地玩在一起。怎、怎么看都是…

**派蒙：**#你们两个…我是{NICKNAME}的向导兼伙伴啦！才不是主人和宠物的关系。

**马科拉：**原来如此，是伙伴的关系么？果然「伙伴」这个词听上去更有韵味一些，你觉得呢安妮兹卡？

**安妮兹卡：**嗯！从现在开始，博比克就是我们的伙伴了！

**马科拉：**既然已经赶跑了不礼貌的家伙，我们就按之前的计划，继续去创作艺术吧？博比克也会高兴的。

**安妮兹卡：**刚才我就觉得博比克很有艺术天分，搬箱子的时候，我看它都激动得颤抖了。

**马科拉：**#向导小姐和{NICKNAME}，也一起来吧？毕竟大家都已经是一起打过架的伙伴了。

**分支 1：**

  **旅行者：**当然，打架最好玩了。

  **派蒙：**你…不过，你也是这么觉得的吧？愚人众的那些家伙大概不会善罢甘休的，如果博比克落入他们手里就不好了。

  **安妮兹卡：**刚才博比克好像有些被吓到了，我要和马科拉排查一下它是不是一切都好。

  **马科拉：**剩下的箱子还有三处，就拜托向导小姐带领了。我们稍后来跟你们汇合。

**分支 2：**

  **旅行者：**和伙伴们一起创作艺术很有趣！

  ↩ 合流到节点 `760040917`



#### 对话 7600422

**起点节点 `760042201`**


**安妮兹卡：**博比克不要怕哦！我们会保护你的！

**马科拉：**嗯嗯，让我看看前爪…好像一切正常呢！




### 步骤 10：子任务 7600430

前往指定地点，继续「创作艺术」

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600442,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11369:1834.240,236.120,9236.053]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600442

**起点节点 `760044201`**


**[黑屏]** 稍微等了一会，就能听到博比克的脚步声往这里来了。




### 步骤 11：子任务 7600410

前往指定地点，继续「创作艺术」

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600410"
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_11369:1834.240,236.120,9236.053]",
      "30"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600422

**起点节点 `760042201`**


**安妮兹卡：**博比克不要怕哦！我们会保护你的！

**马科拉：**嗯嗯，让我看看前爪…好像一切正常呢！



#### 对话 7600426

**起点节点 `760042601`**


**安妮兹卡：**哇！博比克又有新玩具了，真好！

**马科拉：**嗯，真是幸福的孩子。



#### 对话 7600432

**起点节点 `760043201`**


**安妮兹卡：**虽然我不是很懂艺术品，但这也太漂亮了吧！让我好好欣赏一下。

**马科拉：**真的，越看越有味道！




### 步骤 12：子任务 7600435

前往指定地点，继续「创作艺术」

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600443,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607049"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11370:1966.383,262.619,9200.492,0.000,297.295,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600443

**起点节点 `760044301`**


**[黑屏]** 不一会儿，安妮兹卡就领着马科拉和博比克来到了这里。




### 步骤 13：子任务 7600411

前往指定地点，继续「创作艺术」

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600411"
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11370:1966.383,262.619,9200.492,0.000,297.295,0.000]",
      "30"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600426

**起点节点 `760042601`**


**安妮兹卡：**哇！博比克又有新玩具了，真好！

**马科拉：**嗯，真是幸福的孩子。



#### 对话 7600432

**起点节点 `760043201`**


**安妮兹卡：**虽然我不是很懂艺术品，但这也太漂亮了吧！让我好好欣赏一下。

**马科拉：**真的，越看越有味道！



#### 对话 7600435

**起点节点 `760043501`**


**马科拉：**#我很期待{NICKNAME}的最新创作！

**安妮兹卡：**我也是！快点快点，我已经等不及想看啦！



#### 对话 7600436

**起点节点 `760043601`**


**马科拉：**噢！这可真是意想不到的摆法！

**安妮兹卡：**太有创意了！




### 步骤 14：子任务 7600441

前往指定地点，继续「创作艺术」

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600441,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11371:1937.100,254.954,9338.395,0.000,195.491,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 15：子任务 7600442

击败愚人众

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600442"
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_11371:1937.100,254.954,9338.395,0.000,195.491,0.000]",
      "30"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 16：子任务 7600443

前往指定地点，继续「创作艺术」

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600445,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11371:1937.100,254.954,9338.395,0.000,195.491,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600445

**起点节点 `760044501`**


**[黑屏]** 过了没一会儿，安妮兹卡一行如约而至。




### 步骤 17：子任务 7600412

前往指定地点，继续「创作艺术」

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600412"
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11371:1937.100,254.954,9338.395,0.000,195.491,0.000]",
      "30"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600435

**起点节点 `760043501`**


**马科拉：**#我很期待{NICKNAME}的最新创作！

**安妮兹卡：**我也是！快点快点，我已经等不及想看啦！



#### 对话 7600436

**起点节点 `760043601`**


**马科拉：**噢！这可真是意想不到的摆法！

**安妮兹卡：**太有创意了！



#### 对话 7600439

**起点节点 `760043901`**


**安妮兹卡：**是要休息一下吗？嗯，四处走走或许会有灵感哦！

**马科拉：**我们就在这儿等你！




### 步骤 18：子任务 7600433

(test)废弃$HIDDEN

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600410,
        3
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_QUEST_STATE_EQUAL",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600411,
        3
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_QUEST_STATE_EQUAL",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600412,
        3
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_QUEST_STATE_EQUAL",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [],
    "NOCCOBOFKHP": 0,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NONE"
  }
}
```

（此步骤无对话）


### 步骤 19：子任务 7600413

与马科拉他们对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600413,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_11460:1907.773,245.546,9235.938]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600413

**起点节点 `760041301`**


**安妮兹卡：**装置艺术这下就全都完成了吧？虽然我不是很懂，但这也太漂亮了吧！

**马科拉：**安妮兹卡参与创作的艺术当然很美！必须要合影留念纪念一下。

**派蒙：**还要留影的吗？

**马科拉：**那当然，毕竟是伙伴们一起完成的艺术品。得找一个风景优美、又能俯瞰到所有装置艺术的地方才行。

**安妮兹卡：**好棒！我来试试能不能让博比克摆出神奇的姿势！欸，等一下，那边好像有什么动静？

**派蒙：**是刚才的那些家伙们！肯定是冲着博比克来的！

**安妮兹卡：**那些绕着博比克飞来飞去的东西是什么？看上去好可疑！

**愚人众特遣队：**哼哼，果然有用！快将陆巡舰带回驻地交给阿洛契卡大人复命！至于你们这几个捣乱的家伙嘛…

**安妮兹卡：**博、博比克？你怎么跟着那些坏蛋们…我们不是刚刚成为了伙伴吗？

**派蒙：**一定是愚人众用了什么坏手段，拐跑了博比克！

**安妮兹卡：**得赶紧追上他们！把博比克从坏蛋手里夺回来！

**马科拉：**哼，挡路的家伙，真是麻烦！



#### 对话 7600439

**起点节点 `760043901`**


**安妮兹卡：**是要休息一下吗？嗯，四处走走或许会有灵感哦！

**马科拉：**我们就在这儿等你！




### 步骤 20：子任务 7600414

追上被夺走的博比克

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600414"
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600432,
        3
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_QUEST_STATE_EQUAL",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "133607052",
        "0"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_UNREGISTER_DYNAMIC_GROUP"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11372:1902.597,245.044,9238.468]",
      "15"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 21：子任务 7600415

追上被夺走的博比克

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600415,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    },
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600432,
        3
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_QUEST_STATE_EQUAL",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607069"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11373:1895.224,235.400,9324.979]",
      "30"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 22：子任务 7600432

(test)隐藏$HIDDEN

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600432,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600414,
        3
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_QUEST_STATE_EQUAL",
      "PPFLKMOMKAI": ""
    }
  ],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [],
    "NOCCOBOFKHP": 0,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_NONE"
  }
}
```

（此步骤无对话）


### 步骤 23：子任务 7600434

追上被夺走的博比克

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600415,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607069,133607266"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_11373:1895.224,235.400,9324.979]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600415

**起点节点 `760041501`**


**派蒙：**是博比克！他们就在前面！

**愚人众特遣队：**这是最后的警告，你们的宠物现在已经成为了愚人众的猛兽，再往前一步，就会把你们撕得粉碎！

**安妮兹卡：**你对博比克做了什么？！你没有听到吗？它现在很痛苦！快放手！

**愚人众特遣队：**听到了吗？陆巡舰，让你放手呢。




### 步骤 24：子任务 7600438

追上被夺走的博比克

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600438,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607069"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_11373:1895.224,235.400,9324.979]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 25：子任务 7600444

追上被夺走的博比克

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600453,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_11373:1895.224,235.400,9324.979]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600453

**起点节点 `760045301`**


**[黑屏]** 躲开攻击后，发现愚人众他们已经撤入了实验室内。




### 步骤 26：子任务 7600427

追上被夺走的博比克

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600427,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_12195:1955.610,229.049,9350.789,0.000,122.156,0.000]",
      "10"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 27：子任务 7600416

与马科拉他们商议对策

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600416,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_11375:1945.945,230.091,9357.812]",
      "10"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600416

**起点节点 `760041601`**


**马科拉：**呼、呼…差了一点点，还是没有追上…

**安妮兹卡：**这下可麻烦了，毕竟我们是宝藏猎人，不是怪盗，开门可不是我们的强项…

**马科拉：**以德兹斯拉沃的谨慎，生产和维修博比克的实验室肯定会用各种高科技手段锁住大门，正面突破恐怕也行不通…

**分支 1：**

  **旅行者：**他没有留下钥匙给你们吗？

  **派蒙：**对哦，既然德兹斯拉沃是你们的好朋友，还把博比克托付给你们，那肯定很信任你们了。你们应该也会有他实验室的钥匙吧？

  **马科拉：**很可惜，并没有。我们只是一起在「旗舰」喝了八杯酒的那种好朋友。

  **安妮兹卡：**都怪马科拉，当时要是再多喝一杯的话，就能变成真正的至交了吧！

  **派蒙：**…一起喝了八杯酒就是好朋友了吗？那一起揍过愚人众的我们，岂不已经是生死之交了！

  **安妮兹卡：**生死、之交…好棒！再没有比这个词更确切的了！我就知道你们一定会和我们一起冲进去，从坏蛋手里解救出博比克的！

  **派蒙：**现在不是还没有想到冲进去的办法吗？

  **旅行者：**实验室应该会有通风管道才对。

  **派蒙：**好有道理！既然是生产和维修大型机械装置的实验室，没有通风管道一定会被呛晕的吧？我们去洞穴的上方调查看看？

  **马科拉：**说起来，前段时间那边一直有人在鼓捣着些什么，动静不小…现在回想起来，该不会就是他们想闯进实验室吧？

  **安妮兹卡：**肯定就是这样没错！怪不得小德说最近他遇上了些麻烦，得离开家一段时间。我们快出去看看吧！

**分支 2：**

  **旅行者：**有没有可能存在别的路径？

  **派蒙：**既然德兹斯拉沃是你们的好朋友，还把博比克托付给你们，那肯定很信任你们了。或许会把秘密通路之类的情报也告诉你们？

  ↩ 合流到节点 `760041608`




### 步骤 28：子任务 7600417

找到实验室的通风管道

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600420,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11376:1979.823,273.141,9311.146]",
      "40"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600420

**起点节点 `760042001`**


**安妮兹卡：**好像有一个巨大的箱子，堵住了洞穴的入口。

**马科拉：**之前好像没有这些东西…看起来，这里就是实验室的正上方了。只要挪开这只箱子应该就能进去！

**安妮兹卡：**博比克，要是你在的话就好了…

**马科拉：**这么大的箱子，肯定不是人能搬得动的。或许那些家伙就是用吊臂把箱子丢过来的，那也可以<color=#00E1FFFF>用吊臂挪走</color>吧？

**派蒙：**我们快过去调查一下吧。



#### 对话 7600446

**起点节点 `760044601`**


**安妮兹卡：**好担心博比克啊…

**马科拉：**#博比克不会有事的，有{NICKNAME}的帮助，我们肯定能救出它的！




### 步骤 29：子任务 7600418

进入实验室

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600418"
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607044"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11377:2025.747,288.294,9305.180]",
      "20"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 30：子任务 7600431

进入实验室

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600431,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_360700175:2021.385,269.466,9307.238]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 31：子任务 7600419

与马科拉他们对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600402,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11378:2032.872,233.619,9382.361,0.000,186.588,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600402

**起点节点 `760040201`**


**马科拉：**实验室就在下面，当然，还有那些拐走博比克的坏家伙们。

**愚人众特遣队：**阿洛契卡大人，设备已回收，正在按照您的指示准备改造。

**阿洛契卡：**哼，所以说科学家就是不值得信任。自以为掌握了某种程度的「真理」，就开始妄想凌驾于秩序之上，违抗指令，自行其是。

**阿洛契卡：**将如此强大的、足以改写战局的利器，设计成了民用搬运机…这算什么？坚守着可笑的信条？还是在逃避自己手中的力量？

**阿洛契卡：**可惜，世界不是靠一厢情愿运转的。力量就该被合理利用，而不是浪费在这些无意义的伪善上。

**阿洛契卡：**放心，陆巡舰，你很快就可以拥有本就属于你的力量了。

**马科拉：**唔！看样子，他们要对博比克下手了…

**安妮兹卡：**可恶的家伙！我们可以冲进去了吧？现在就冲进去吧？

**分支 1：**

  **旅行者：**夺回博比克！

  **安妮兹卡：**还要好好教训那些坏家伙们！

**分支 2：**

  **旅行者：**博比克是全地形民用运输装置！

  ↩ 合流到节点 `760040211`



#### 对话 7600446

**起点节点 `760044601`**


**安妮兹卡：**好担心博比克啊…

**马科拉：**#博比克不会有事的，有{NICKNAME}的帮助，我们肯定能救出它的！



#### 对话 7600452

**起点节点 `760045201`**


**安妮兹卡：**呜啊啊——我们冲吧！我快等不及了！

**马科拉：**安妮兹卡你先不要冲动！要跟我们的生死之交一起行动呀！




### 步骤 32：子任务 7600445

击败愚人众

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600445"
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_12184:2017.730,213.807,9381.053]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 33：子任务 7600436

击败愚人众

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600436"
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_11379:2014.978,213.603,9366.336]",
      "50"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 34：子任务 7600421

将博比克恢复成正常状态

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600421,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_11380:2004.651,229.369,9340.240]",
      "50"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600421

**起点节点 `760042101`**


**安妮兹卡：**马科拉，你们还好吗？

**马科拉：**我们都没事！愚人众的坏蛋已经都被我们解决啦！

**安妮兹卡：**看！我发现了改写博比克的控制台！看上去很复杂的样子。

**派蒙：**安妮兹卡，你先不要操控，等我们过来——

**安妮兹卡：**你们说了什么？我听不清！我刚才说因为控制台太复杂了，我就凭悟性按了按钮，没想到真的终止了改写程序！

**派蒙：**谢、谢天谢地…

**安妮兹卡：**不过！虽然终止了程序，但好像整个控制台完全卡死了，接下来不管我按什么按键都没有反应了！要怎么办呢？

**派蒙：**欸？就这么乱按的吗！万一刚好激活了什么自毁程序可怎么办啊！

**马科拉：**别、别急。我想德兹斯拉沃留下的这本说明文档里，一定提到过遇到这种情况该怎么办，让我找一找…

**分支 1：**

  **旅行者：**再没有比通读说明文档更妥当的了。

  **马科拉：**我找到了！「如果遇到程序模组改写失败导致控制台卡死时，需手动重启能源，并接入<color=#00E1FFFF>备份的程序模组</color>到控制台！」

  **马科拉：**重启能源应该很容易吧？只要找到像开关一样的东西就可以了。但是这个「备份程序模组」究竟是什么呢…

  **派蒙：**#没关系，交给{NICKNAME}的悟性，一切都能解决的！

  **马科拉：**说得没错，既然是说明文档不能解决的问题，当然应该交给悟性了！我也会再找找看有没有其他的线索的。

**分支 2：**

  **旅行者：**有句格言，似乎是重启解决之类。

  **派蒙：**#咦？我怎么没有听说过这句格言？是{NICKNAME}用悟性悟出来的格言吗？

  ↩ 合流到节点 `760042113`



#### 对话 7600452

**起点节点 `760045201`**


**安妮兹卡：**呜啊啊——我们冲吧！我快等不及了！

**马科拉：**安妮兹卡你先不要冲动！要跟我们的生死之交一起行动呀！



#### 对话 7600462

**起点节点 `760046201`**


**马科拉：**那么就拜托你调查一下了！我来翻翻小德留下的文档，或许会有别的线索…




### 步骤 35：子任务 7600422

寻找重启能源的开关

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600422"
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607063"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11406:2001.074,229.181,9339.639]",
      "10"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 36：子任务 7600437

寻找重启能源的开关

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600427,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607063,133607064"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_11380:2004.651,229.369,9340.240]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600427

**起点节点 `760042701`**


**陌生的声音：**唉，健忘的我自己。没错这个就是重启能源的开关。控制台系统又卡死了吧？如果要重刷博比克，记得要找到备份程序模组。

**派蒙：**好像成功了，接下来只要找到备份程序模组就可以了！

**马科拉：**刚才那个声音就是德兹斯拉沃！听起来就仿佛他本人正在这里，好有意思。德兹斯拉沃不愧是天才科学家。

**马科拉：**看样子他留了不少提醒自己的小贴士。太好了，这样找线索应该就容易多了。感谢他糟糕的记忆力！



#### 对话 7600462

**起点节点 `760046201`**


**马科拉：**那么就拜托你调查一下了！我来翻翻小德留下的文档，或许会有别的线索…




### 步骤 37：子任务 7600423

寻找备份程序模组

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600423"
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11405:2013.916,219.183,9366.626]",
      "50"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600454

**起点节点 `760045401`**


**安妮兹卡：**博比克…稍微等一下，马上就救你出来！



#### 对话 7600455

**起点节点 `760045501`**


**马科拉：**唔，说明文档里也没有写备份程序模组到底长什么样子啊…



#### 对话 7600466

**起点节点 `760046601`**


**调查：**「今天在找东西这件事上浪费了二十六分钟的时间…」

**调查：**「看来有必要将实验用品按编号分类归置，嗯，下一项工作就来设计一张归置记录表吧。」

**派蒙：**这大概是那个科学家用来提醒自己的便条吧？反面还有一行字呢！

**调查：**「备忘：要是备份程序模组不在这里，就在平台上找找！那附近摆了有箭头指着的显像机关！」

**派蒙：**看来那个什么模组在<color=#00E1FFFF>摆了带箭头显像机关的平台</color>那边，我们去找找看吧。




### 步骤 38：子任务 7600440

寻找备份程序模组

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600431,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_12698:2018.808,220.542,9354.781,0.000,197.410,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600431

**起点节点 `760043101`**


**派蒙：**找到了，这个就是备份程序模组了吧！

**马科拉：**太棒了！不愧是兼备科学和艺术的头脑。按照说明文档的指示，只要把备份程序模组插进控制台里就好了！



#### 对话 7600454

**起点节点 `760045401`**


**安妮兹卡：**博比克…稍微等一下，马上就救你出来！



#### 对话 7600455

**起点节点 `760045501`**


**马科拉：**唔，说明文档里也没有写备份程序模组到底长什么样子啊…




### 步骤 39：子任务 7600424

与马科拉一起，将备份程序模组插进控制台

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 1,
      "LNHLPKELCAL": [
        0,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_LUA_NOTIFY",
      "PPFLKMOMKAI": "Q7600424"
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607065"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    },
    {
      "LNHLPKELCAL": [
        "121368",
        "1"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_DEL_PACK_ITEM"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11387:2009.597,226.235,9389.103]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 40：子任务 7600425

与马科拉对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600425,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_11388:2012.308,225.376,9392.385,0.000,205.586,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600425

**起点节点 `760042501`**


**德兹斯拉沃的声音：**看来你找到了备份程序模组，那就开始吧！希望写入顺利，不然的话可就头疼了…

**派蒙：**博比克，全地形民用运输装置，请一定要恢复起来呀！

**马科拉：**博比克，德兹斯拉沃最骄傲的发明，请一定要恢复起来呀！

**安妮兹卡：**博比克，安妮兹卡和马科拉的好伙伴，请一定要恢复起来呀！

**德兹斯拉沃的声音：**备份程序模组写入已完成。这里还可以手动写入其他参数，别忘记了。

**派蒙：**手动写入其他参数？这是什么意思？

**马科拉：**说明文档上说，如果在这里手动设置参数的话，会写进博比克的底层系统里去呢。不过我觉得默认参数就够用了吧？

**安妮兹卡：**等一下！也就是说，不管在这里偷偷加上什么参数，博比克都会忠实执行的对吧…

**安妮兹卡：**那，能不能写上——博比克尊安妮兹卡为提瓦特的女王，誓死捍卫安妮兹卡和马科拉，我们三个是最好的朋友，永远也不分开？

**派蒙：**不行啦！输入这么多奇怪的东西，博比克会坏掉的！

**派蒙：**#博比克的备份程序模组是{NICKNAME}找到的，不如就让{M#他}{F#她}决定吧。

**分支 1：**

  **旅行者：**什么也不加。

  **派蒙：**嗯，我也觉得这样是最好的。博比克就是博比克！

  **派蒙：**嗯！既然设定好了参数，就按下确认键吧。

  **德兹斯拉沃的声音：**博比克-131，全地形民用运输装置，启动！

**分支 2：**

  **旅行者：**博比克是人类的好朋友。

  **安妮兹卡：**不可以是「安妮兹卡的好朋友」吗？只多了两个字而已…

  ↩ 合流到节点 `760042518`

**分支 3：**

  **旅行者：**大幅强化博比克的搬运能力。

  **马科拉：**噢噢，真是令人期待呢！

  ↩ 合流到节点 `760042518`




### 步骤 41：子任务 7600426

跟随博比克前往实验室入口

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600426,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607059"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_POINT",
    "LNHLPKELCAL": [
      "[3_11483:1992.870,230.272,9333.272]",
      "15"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 42：子任务 7600428

与马科拉他们对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600428,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607067"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NO_PILLAR",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_11483:1992.870,230.272,9333.272]",
      "15"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600428

**起点节点 `760042801`**


**派蒙：**大门还是紧闭的状态呢，刚才也没有找到钥匙之类的东西。估计是在德兹斯拉沃手上吧。

**安妮兹卡：**但现在情况已经完全不一样了，我们找回了我们的伙伴。

**马科拉：**没错，博比克的话，一定能打开这扇门的。

**安妮兹卡：**#{NICKNAME}，博比克最听你的话了，不如就由你来下达这个指令吧！

**旅行者：**博比克，展现你的力量，开启大门。

**安妮兹卡：**嗯，交给你了博比克！带我们离开这里吧。




### 步骤 43：子任务 7600429

与马科拉他们对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600448,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607266"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_SCENE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_TARGET",
    "LNHLPKELCAL": [
      "[3_11395:1898.533,235.374,9338.601,0.000,202.692,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600448

**起点节点 `760044801`**


**安妮兹卡：**#终于出来了！多亏了{NICKNAME}和博比克！

**马科拉：**安妮兹卡也很棒哦！之后一定要好好教我怎么操作博比克才行。

**安妮兹卡：**你不是已经认真读完了小德的说明文档了嘛？应该是你教我才对吧？

**马科拉：**#已经有安妮兹卡和{NICKNAME}两位大师了，那个东西不读也罢。

**安妮兹卡：**噢！这里正好有不少箱子，不如来试一试博比克恢复得怎么样了吧？




### 步骤 44：子任务 7600420

与马科拉他们对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600420,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_FINISH_PLOT",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_11395:1898.533,235.374,9338.601,0.000,202.692,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```

（此步骤无对话）


### 步骤 45：子任务 7600439

与马科拉他们对话

**任务条件：**

```json
{
  "finishCond": [
    {
      "BFGALAINLMD": 0,
      "LNHLPKELCAL": [
        7600450,
        0
      ],
      "PAINLIBBLDK": "QUEST_CONTENT_COMPLETE_TALK",
      "PPFLKMOMKAI": ""
    }
  ],
  "triggerCond": [],
  "execActions": [
    {
      "LNHLPKELCAL": [
        "3",
        "133607023"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_NOTIFY_GROUP_LUA"
    },
    {
      "LNHLPKELCAL": [
        "7600401",
        "1"
      ],
      "PAINLIBBLDK": "QUEST_EXEC_INC_QUEST_GLOBAL_VAR"
    }
  ],
  "guide": {
    "BEALNDJFMMC": "QUEST_GUIDE_IN_SCENE_STYLE_NONE",
    "BJKFMDGPIBN": "QUEST_GUIDE_INDICATOR_STYLE_NONE",
    "CCLPMCLNLBJ": "NONE",
    "CJHEDKCIHPA": 0,
    "EMAIGPLHEJH": "QUEST_GUIDE_LAYER_NONE",
    "GGJOHCAJAPP": 0,
    "HFNCMGLBNGO": "",
    "IDLNFDCEJLG": "QUEST_GUIDE_AREA_STYLE_DEFAULT",
    "KCFINIEBBBG": 0,
    "KEDGDANLBHP": "",
    "KNMAMMFJAOB": "QUEST_GUIDE_STYLE_NONE",
    "LNHLPKELCAL": [
      "[3_11394:1896.517,235.555,9339.354,0.000,181.344,0.000]"
    ],
    "NOCCOBOFKHP": 3,
    "OFEBJNGBEPL": "QUEST_GUIDE_AUTO_NONE",
    "PAINLIBBLDK": "QUEST_GUIDE_LOCATION"
  }
}
```


#### 对话 7600430

**起点节点 `760043001`**


**派蒙：**不、不可思议，博比克的力量竟然比之前增强了那么多！刚才好像听到了箱子掉进海里的声音…

**安妮兹卡：**好棒好棒好棒！不愧是我的博比克！

**安妮兹卡：**#这次真的是太感谢你们了，{NICKNAME}、派蒙！光靠我们两个肯定救不了博比克！

**马科拉：**现在博比克也恢复了，比以前更厉害了。下次再遇到麻烦，我们也能更好地应对。

**安妮兹卡：**对了，我们不是说好要合影留作纪念吗？结果刚才那么混乱，差点忘记了！

**马科拉：**没错，要找个风景秀丽的地方，把博比克也一起拍进去！

**[黑屏]** 来到实验室外一个光影相当不错的地方，是绝佳的合影场所。

**马科拉：**这个地方很不错！那么，准备好了吗？要合影了哦！

**安妮兹卡：**三、二、一，博比——克！



#### 对话 7600450

**起点节点 `760045001`**


**派蒙：**不、不可思议，博比克的力量竟然比之前增强了那么多！刚才好像听到了箱子掉进海里的声音…

**安妮兹卡：**好棒好棒好棒！不愧是我的博比克！

**安妮兹卡：**#这次真的是太感谢你们了，{NICKNAME}、派蒙！光靠我们两个肯定救不了博比克！

**马科拉：**现在博比克也恢复了，比以前更厉害了。下次再遇到麻烦，我们也能更好地应对。

**安妮兹卡：**对了，我们不是说好要合影留作纪念吗？结果刚才那么混乱，差点忘记了！

**马科拉：**没错，要找个风景秀丽的地方，把博比克也一起拍进去！

**[黑屏]** 来到实验室外一个光影相当不错的地方，是绝佳的合影场所。

**马科拉：**这个地方很不错！那么，准备好了吗？要合影了哦！

**安妮兹卡：**三、二、一，博比——克！


